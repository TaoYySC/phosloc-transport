import torch
import torch.nn as nn
from torch_geometric.nn import GATConv, global_mean_pool, global_add_pool


class GNNEncoder(nn.Module):
    def __init__(
        self,
        node_input_dim,
        hidden_dim=64,
        output_dim=64,
        heads=4,
        dropout=0.3,
        edge_dim=2,
    ):
        super().__init__()

        self.conv1 = GATConv(
            in_channels=node_input_dim,
            out_channels=hidden_dim,
            heads=heads,
            dropout=dropout,
            edge_dim=edge_dim,
        )

        self.conv2 = GATConv(
            in_channels=hidden_dim * heads,
            out_channels=output_dim,
            heads=1,
            dropout=dropout,
            edge_dim=edge_dim,
        )

        self.act = nn.ReLU()
        self.dropout = nn.Dropout(dropout)

        self.fuse = nn.Sequential(
            nn.Linear(output_dim * 2, output_dim),
            nn.ReLU(),
            nn.Dropout(dropout),
        )

    def forward(self, data):
        edge_attr = getattr(data, "edge_attr", None)

        x = self.conv1(data.x, data.edge_index, edge_attr=edge_attr)
        x = self.act(x)
        x = self.dropout(x)

        x = self.conv2(x, data.edge_index, edge_attr=edge_attr)
        x = self.act(x)
        x = self.dropout(x)

        g_mean = global_mean_pool(x, data.batch)

        center_mask = data.center_mask.view(-1, 1).to(dtype=x.dtype, device=x.device)
        center_sum = global_add_pool(x * center_mask, data.batch)
        center_count = global_add_pool(center_mask, data.batch).clamp(min=1.0)
        g_center = center_sum / center_count

        g = torch.cat([g_center, g_mean], dim=1)
        g = self.fuse(g)

        return g


class ESMCNN2DSiteGNNClassifier(nn.Module):
    def __init__(
        self,
        input_dim=1280,
        proj_input_dim=64,
        conv_channels=(32, 64),
        seq_kernel_size=3,
        encoder_output_dim=64,
        site_hidden_dims=(256, 128),
        site_output_dim=64,
        gnn_node_input_dim=23,
        gnn_hidden_dim=64,
        gnn_output_dim=64,
        fusion_hidden_dims=(128, 64),
        dropout=0.3,
    ):
        super().__init__()

        c1, c2 = conv_channels

        self.input_proj = nn.Sequential(
            nn.Linear(input_dim, proj_input_dim),
            nn.ReLU(),
            nn.Dropout(dropout),
        )

        self.feature_extractor = nn.Sequential(
            nn.Conv2d(
                in_channels=1,
                out_channels=c1,
                kernel_size=(seq_kernel_size, proj_input_dim),
                padding=(seq_kernel_size // 2, 0),
            ),
            nn.ReLU(),
            nn.BatchNorm2d(c1),
            nn.Conv2d(
                in_channels=c1,
                out_channels=c2,
                kernel_size=(seq_kernel_size, 1),
                padding=(seq_kernel_size // 2, 0),
            ),
            nn.ReLU(),
            nn.BatchNorm2d(c2),
        )

        self.pool = nn.AdaptiveAvgPool2d((1, 1))

        self.window_proj = nn.Sequential(
            nn.Linear(c2, encoder_output_dim),
            nn.ReLU(),
            nn.Dropout(dropout),
        )

        s1, s2 = site_hidden_dims

        self.site_encoder = nn.Sequential(
            nn.Linear(input_dim, s1),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(s1, s2),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(s2, site_output_dim),
            nn.ReLU(),
            nn.Dropout(dropout),
        )

        self.gnn_encoder = GNNEncoder(
            node_input_dim=gnn_node_input_dim,
            hidden_dim=gnn_hidden_dim,
            output_dim=gnn_output_dim,
            heads=4,
            dropout=dropout,
            edge_dim=2,
        )

        fusion_in_dim = encoder_output_dim + site_output_dim + gnn_output_dim
        f1, f2 = fusion_hidden_dims

        self.classifier = nn.Sequential(
            nn.Linear(fusion_in_dim, f1),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(f1, f2),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(f2, 1),
        )

    def encode(self, features):
        x = features["esm"]
        graph = features["af_graph"]

        center_idx = x.size(1) // 2
        x_site = x[:, center_idx, :]

        x_window = self.input_proj(x)
        x_window = x_window.unsqueeze(1)
        x_window = self.feature_extractor(x_window)
        x_window = self.pool(x_window).squeeze(-1).squeeze(-1)
        z_window = self.window_proj(x_window)

        z_site = self.site_encoder(x_site)
        z_graph = self.gnn_encoder(graph)

        return torch.cat([z_window, z_site, z_graph], dim=1)

    def forward(self, features):
        z = self.encode(features)
        logits = self.classifier(z).squeeze(1)
        return logits

    def forward_with_repr(self, features, n_repr_layers=2):
        from src.models.classifier_repr import run_classifier_with_repr

        z = self.encode(features)
        return run_classifier_with_repr(self.classifier, z, n_repr_layers=n_repr_layers)